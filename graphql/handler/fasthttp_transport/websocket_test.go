...
func TestWebsocket(t *testing.T) {
	handler := testserver.NewFast()
	handler.AddTransport(transport.Websocket{})

	lh := `localhost:` + randomPort()

	go func() {
		if err := fasthttp.ListenAndServe(lh, handler.ServeHTTP); err != nil {
			panic(err)
		}
...
// 	t.Run("server acks init", func(t *testing.T) {
// 		c := wsConnectWithSubprocotol(lh, graphqltransportwsSubprotocol)
// 		defer c.Close()
