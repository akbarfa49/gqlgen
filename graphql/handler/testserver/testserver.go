...
func NewFast() *FastTestServer {
	...
	srv.FastServer = handler.NewFastServer(&graphql.ExecutableSchemaMock{
		ExecFunc: func(ctx context.Context) graphql.ResponseHandler {
			rc := graphql.GetOperationContext(ctx)
			switch rc.Operation.Operation {
			case ast.Query:
...
// NewError provides a server for use in resolver error tests that isn't relying on generated code. It isnt a perfect reproduction of
// a generated server, but it aims to be good enough to test the handler package without relying on codegen.
func NewFastError() *FastTestServer {
	next := make(chan struct{})
...
type FastTestServer struct {
	*handler.FastServer
	next       chan struct{}
	complexity int
}

func (s *FastTestServer) SendNextSubscriptionMessage() {
	select {
	case s.next <- struct{}{}:
	case <-time.After(1 * time.Second):
		fmt.Println("WARNING: no active subscription")
...
func (s *FastTestServer) SetCalculatedComplexity(complexity int) {
	s.complexity = complexity
}
